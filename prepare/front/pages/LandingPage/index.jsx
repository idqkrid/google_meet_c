...
const socket = io('http://13.125.251.86:3003', {
...
const LandingPage = () => {
  ...
  const handleShareScreen = async () => {
    try {
      ...
      const senders = myPeerConnection.getSenders().find((sender) => sender.track.kind === 'video');
  ...
  return (
    <Block>
      ...
      <StateChnage>
        <StyledButtonButton ref={muteRef} onClick={handleMuteClick}>
          [소리 ON / 소리 OFF]
        </StyledButtonButton>
        <StyledButtonButton ref={cameraRef} onClick={handleCameraClick}>
          [카메라 ON / 카메라 OFF]
        </StyledButtonButton>
        <StyledButtonButton ref={shareScreenRef} onClick={handleShareScreen}>
          [화면 공유하기]
        </StyledButtonButton>
      </StateChnage>
    </Block>
